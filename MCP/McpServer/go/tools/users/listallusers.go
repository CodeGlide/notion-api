...
func ListallusersHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v1/users%s", cfg.BaseURL, queryString)
		...
		var result models.PaginatedUsers
...
func CreateListallusersTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_users",
	...
	return models.Tool{
		Definition: tool,
		Handler:    ListallusersHandler(cfg),
	}
}
